...
def get_page(pageindex,proxies):
    #headers_tmp=dict(headers)
    #headers_tmp['Referer'] = 'http://10.231.18.25/CITYGRID.QUERY/archivesinfo_flat/SearchResultAllFlat.aspx?IsQuery=1'
    ...
    rt=requests.post(url,data=body,headers=headers,proxies=proxies) 
    return rt.content
...
def get_data():
    send_query_condition()
    content = get_page(3,proxies)
    rows,total = parse_page(content)
    rows = filter_has(rows)
    for row in rows:
        row.append( get_coord(row[2]))
