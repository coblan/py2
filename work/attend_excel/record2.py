...
from attend import get_attend_list,AttendRecord
...
class RecordModel(Model): 
    ...
    main_process() ，遍历整个考勤记录，调用attend接口函数，计算考勤记录的其他信息。最后保存
    
    gen07() ,将数据库保存为excel07文件
    
    """
    
    attend_number=CharField(default='')
    name = CharField(default='')
    department = CharField(default='')
    record_date = DateField(default='')
   ...
   #保存的工作日字典
    ...
    def initFromList(cls,ls):
        "ls = [ [attend_number,name,department,record_date,workstart,workleave],]"
        conn = sqlite3.connect(':memory:')
        RecordModel.connection(conn)
        RecordModel.create()  
        for row in ls:
            dc ={"attend_number":row[0],
                "name":row[1],
                "department":row[2],
                "record_date":row[3],
                "workstart":row[4],
    ...
    def main_process(cls):
        ...
        for p in RecordModel.select():
            ...
            p.workshift = TmsData.kao2workshift(p.attend_number)
            record = AttendRecord(attend_number=p.attend_number,workstart= str(p.workstart), workleave=str(p.workleave), record_date=p.record_date.strftime("%Y/%m/%d"), \
    ...
    def gen07(cls, path):
        ...
        for row in RecordModel.select("ORDER BY attend_number ,record_date"):
            
            assert isinstance(row,RecordModel)

            # 整理格式准备写入excel2007
            outrow=[row.attend_number,row.name,row.department,row.record_date.strftime("%Y/%m/%d"),row.workshift,row.workstart,row.workleave,row.note,row.sub_sequence,row.late_team,row.workspan,row.late_person,row.overtime,row.early_leave]
    ...
    def day_type(self):
        datestr = self.record_date.strftime("%Y/%m/%d")
        for k,v in self.daystn.items():
            ...
            #elif datestr in v:
                #return k
        raise ValueError("which type %s should be?"%datestr)  
    
def overtime_from_date(attend_number,datestr):
    for i in RecordModel.select("WHERE record_date='%s' AND attend_number='%s'"%(datestr,attend_number)):
